(function() {
  ...
  window._vaSchemas = {};
  window._vaProgress = { processed: 0, total: 0, current: null, errors: [] };
  ...
  async function processForm(form) {
    const name = form.name; const url = form.url;
    if (stopped) throw new Error('Stopped');

    window._vaProgress.current = name;
    const url = form.url;
    ...
    try {
      ...
      window._vaProgress.processed++;
    } catch (e) {
      const msg = name + ': ' + e.message;
      window._vaProgress.errors.push(msg);
      window._vaProgress.processed++;
      console.error('✗ ' + msg);
    }
  }
  ...
  // ─── Entry points ───
  window.startVABatch = async function(forms) {
    await ensurePdfLib();
    stopped = false;
    window._vaSchemas = {};
    window._vaProgress = { processed: 0, total: forms.length, current: null, errors: [] };
    ...
    const p = window._vaProgress;
    console.log('═══════════════════════════════════════');
    console.log(`Done in ${elapsed}s. Processed: ${p.processed}/${p.total}, Errors: ${p.errors.length}`);
    console.log(`Schemas stored in window._vaSchemas (${Object.keys(window._vaSchemas).length} forms)`);
    console.log('═══════════════════════════════════════');
  };

  window.stopVABatch = function() {
    stopped = true;
    console.log('Batch stopped.');
  };

  console.log('VA batch processor loaded. Call startVABatch(forms) to begin.');
})();
